...
class GeneDatasetCompleteness:
    ...
    def plot_percentage_barplot(self, df_percentage, title='Barplot of Percentages'):
        ...
        palettes = ["viridis", "plasma", "cividis", "magma", "crest", "cubehelix", "coolwarm", "Blues", "Greens"]
        chosen_palette = random.choice(palettes)
        
        plt.figure(figsize=(12, 8))
        barplot = sns.barplot(x='Column', y='Percentage', data=df_plot, palette=chosen_palette)
        ...
        plt.tight_layout()
        plt.savefig(f"{os.getcwd()}/{title.replace(' ', '_')}.tiff", dpi=600)
